...
public:
  ...
  LogicalResult matchAndRewrite(tosa::RescaleOp op,
    ...
    SmallVector<int32_t> shiftValues;
    getValuesFromIntArrayAttribute(op.shift(), shiftValues);

    bool doubleRound =
        op.double_round() &&
        llvm::any_of(shiftValues, [](int32_t v) { return v > 31; });
    bool onlyDoubleRound =
        op.double_round() &&
        llvm::all_of(shiftValues, [](int32_t v) { return v > 31; });
    ...
    auto shiftType = RankedTensorType::get(shiftShape, rewriter.getI32Type());
    ...
    auto linalgOp = rewriter.create<linalg::GenericOp>(
        ...
        [&](OpBuilder &nestedBuilder, Location nestedLoc,
          ...
          Value one = nestedBuilder.create<ConstantOp>(
              nestedLoc,
              nestedBuilder.getIntegerAttr(nestedBuilder.getI32Type(), 1));

          Value shiftMinusOne =
              nestedBuilder.create<SubIOp>(nestedLoc, shift, one);

          // Guarantee the result rounds up.
          Value round = nestedBuilder.create<mlir::ShiftLeftOp>(
              nestedLoc,
              nestedBuilder.create<SignExtendIOp>(
                  nestedLoc, nestedBuilder.getI64Type(), one),
              nestedBuilder.create<SignExtendIOp>(
                  nestedLoc, nestedBuilder.getI64Type(), shiftMinusOne));

          // If we need to double round we need to modify the round value.
          if (doubleRound) {
            Value zero = nestedBuilder.create<ConstantOp>(
                nestedLoc,
                nestedBuilder.getZeroAttr(nestedBuilder.getI32Type()));
            Value thirty = nestedBuilder.create<ConstantOp>(
                nestedLoc,
                nestedBuilder.getIntegerAttr(nestedBuilder.getI32Type(), 30));
            Value shiftThirty = nestedBuilder.create<mlir::ShiftLeftOp>(
                nestedLoc,
                nestedBuilder.create<SignExtendIOp>(
                    nestedLoc, nestedBuilder.getI64Type(), one),
                nestedBuilder.create<SignExtendIOp>(
                    nestedLoc, nestedBuilder.getI64Type(), thirty));

            // Compute the round value depending whether the value is positive
            // or negative.
            Value roundAdd = nestedBuilder.create<mlir::AddIOp>(
                nestedLoc, round, shiftThirty);
            Value roundSub = nestedBuilder.create<mlir::SubIOp>(
                nestedLoc, round, shiftThirty);

            Value valueGreaterThanZero = nestedBuilder.create<mlir::CmpIOp>(
                loc, CmpIPredicate::sge, value, zero);

            Value roundAdjust = nestedBuilder.create<mlir::SelectOp>(
                loc, valueGreaterThanZero, roundAdd, roundSub);

            // Double rounding should only occur if the shift value is greater
            // than 31.
            if (onlyDoubleRound) {
              round = roundAdjust;
            } else {
              Value thirtyTwo = nestedBuilder.create<ConstantOp>(
                  nestedLoc,
                  nestedBuilder.getIntegerAttr(nestedBuilder.getI32Type(), 32));
              Value shiftGreaterThanThirtyTwo =
                  nestedBuilder.create<mlir::CmpIOp>(loc, CmpIPredicate::sge,
                                                     shift, thirtyTwo);
              round = nestedBuilder.create<mlir::SelectOp>(
                  loc, shiftGreaterThanThirtyTwo, roundAdjust, round);
            }
          }

          multiplier = nestedBuilder.create<SignExtendIOp>(
              nestedLoc, nestedBuilder.getI64Type(), multiplier);

          value = nestedBuilder.create<SignExtendIOp>(
              nestedLoc, nestedBuilder.getI64Type(), value);
          value = nestedBuilder.create<MulIOp>(nestedLoc, value, multiplier);

          // Apply rounding and downshift.
          value = nestedBuilder.create<AddIOp>(nestedLoc, value, round);
          shift = nestedBuilder.create<mlir::SignExtendIOp>(
              loc, rewriter.getI64Type(), shift);
          value =
              nestedBuilder.create<mlir::SignedShiftRightOp>(loc, value, shift);
          value = nestedBuilder.create<mlir::TruncateIOp>(
              nestedLoc, rewriter.getI32Type(), value);
...
void mlir::tosa::populateTosaToLinalgOnTensorsConversionPatterns(
  ...
  patterns->insert<
      ...
      TransposeConverter, RescaleOpConverter>(context);
}
